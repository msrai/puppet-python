...
define python::virtualenv (
  ...
  if $ensure == 'present' {
    ...
    exec { "python_virtualenv_${venv_dir}":
      command     => "true ${proxy_command} && ${virtualenv_cmd} ${system_pkgs_flag} -p ${python} ${venv_dir} && ${pip_cmd} wheel --help > /dev/null 2>&1 && { ${pip_cmd} wheel --version > /dev/null 2>&1 || wheel_support_flag='--no-binary :all:'; } ; { ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag} \$wheel_support_flag --upgrade pip || ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag}  --upgrade pip ;}",
      user        => $owner,
    ...
    exec { "python_distribute_${venv_dir}":
      command     => "${pip_cmd} wheel --help > /dev/null 2>&1 && { ${pip_cmd} wheel --version > /dev/null 2>&1 || wheel_support_flag='--no-binary :all:'; } ; { ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag} \$wheel_support_flag --upgrade ${distribute_pkg} || ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag}  --upgrade ${distribute_pkg} ;}",
      user        => $owner,
      cwd         => $cwd,
      environment => $environment,
      path        => $path,
    ...
    if $requirements {
      ...
      python::requirements { "${requirements}_${venv_dir}":
        ...
        group          => $group,
        cwd            => $cwd,
        require        => Exec["python_distribute_${venv_dir}"],
